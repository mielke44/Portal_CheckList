...
<v-container grid-list-lg>
    ...
    <v-layout row wrap>
        ...
        <!--FILTRO PENDENTE-->
           <v-expansion-panel v-if="view.filter_type==0" v-model='view.check_resp' readonly>
                    <v-subheader>Tarefas</v-subheader>
                    <v-expansion-panel-content v-for='(c,i) in checks_resp' v-if="(c.status== 0 || c.status==-2)">
                        <template  slot="header">
                            <v-layout row wrap fill-height align-center>
                                ...
                                <v-flex xs5 @click='view.check_resp=i'>
        ...
        <v-expansion-panel v-if="view.filter_type==0" v-model='view.check_group' readonly>
                <v-subheader>Tarefas de grupo</v-subheader>
                <v-expansion-panel-content v-for='(c,i) in checks_group' v-if="(c.status== 0 || c.status==-2)">
                    <template  slot="header">
                        <v-layout row wrap fill-height align-center>
                            <v-flex xs1>
                                <v-checkbox v-if="c.status==-1" color="primary" v-model="c.status"
                            ...
                            <v-flex xs5 @click='view.check_group=i'>
                                @{{get_model(models.task,c.task_id).name}}
                            </v-flex>
                            <v-flex xs5 @click='view.check_group=i'>
                                @{{c.user}}
