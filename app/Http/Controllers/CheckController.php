    ...
    public static function store(Request $request){
        ...
        if(isset($request['id'])){
            ...
            if(isset($request['status'])){
                ...
                if(count($linker_down_coll)>0)foreach($linker_down_coll as $linker_down){
                    ...
                    if(strlen($check_down->resp)==6){
                        ...
                        event(new CheckUpdateEvent($cd,$name,0,$temparray));
                        ...
                        event(new CheckUpdateEvent($cd,$name,$temparray));
                    }else event(new CheckUpdateEvent($cd,$name,array('admin'=>$check_down->resp)));
                }
            }
            ...
            if ($Check->save()) {
                ...
                event(new CheckUpdateEvent($Check,$name,$type,$receiver));
    ...
    public static function createCheck($template_id,$Checklist_id){
        ...
        foreach(ChecklistTemplate::find($template_id)->tasks()->get() as $ct){
            ...
            if(strlen($task->resp)==7){
                ...
                array_push($receiver['admin'],$check->resp);
            }
            $type=2;
            try{
                $check->save();
                ...
                //print_r('notificate check id: '.$check->id."\n");
                //print_r($receiver);
                event(new CheckUpdateEvent($check,$name,2,$receiver));
            }
            catch(Exception $e){
