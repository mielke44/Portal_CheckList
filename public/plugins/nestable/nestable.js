(function (global, factory) {
    ...
    var _objectKeysInternal$1 = function (object, names) {
        ...
        var key;
        for (key in O)
            if (key != IE_PROTO$1) _has$1(O, key) && result.push(key);
        // Don't enum bug & hidden keys
        while (names.length > i)
    ...
    var _setToStringTag = function (it, tag, stat) {
        if (it && !_has$1(it = stat ? it : it.prototype, TAG)) def(it, TAG, {
            configurable: true,
            value: tag
        });
    };
    ...
    ***/
    ...
    // 25.1.2.1.1 %IteratorPrototype%[@@iterator]()
    _hide(IteratorPrototype$1, _wks$1('iterator'), function () {
        return this;
    });
                    ...
                    _this._draggableDestroy = index$1(triggerEl, {
                        ...
                        drop: function drop(e, opt, store$$1) {
                            ...
                            if (_this.store.ondragend && _this.store.ondragend(_this.data, draggableHelperInfo) === false) {
                                ...
                                if (siblings === _this.startPosition.siblings && siblings.indexOf(_this.data) === _this.startPosition.index);
                                else {
